...
h||k.push(f);var l=a.getExtraClassNames();l&&k.push.apply(k,l);if(goog.userAgent.IE&&!goog.userAgent.isVersionOrHigher("7")){var q=this.getAppliedCombinedClassNames_(k);0<q.length&&(k.push.apply(k,q),c=!0)}g&&h&&!l&&!c||goog.dom.classes.set(b,k.join(" "));this.setAriaStates(a,b);return b};goog.ui.ControlRenderer.prototype.initializeDom=function(a){a.isRightToLeft()&&this.setRightToLeft(a.getElement(),!0);a.isEnabled()&&this.setFocusable(a,a.isVisible())};
...
Blockly.BlockSvg.prototype.renderFields_=function(a,b,c){Blockly.RTL&&(b=-b);for(var d=0,e;e=a[d];d++){var f=e.getSize().width;Blockly.RTL?(b-=f,e.getRootElement().setAttribute("transform","translate("+b+", "+c+")"),f&&(b-=Blockly.BlockSvg.SEP_SPACE_X)):(e.getRootElement().setAttribute("transform","translate("+b+", "+c+")"),f&&(b+=f+Blockly.BlockSvg.SEP_SPACE_X))}return Blockly.RTL?-b:b};
Blockly.BlockSvg.prototype.renderCompute_=function(a){var b=this.block_.inputList,c=[];c.rightEdge=a+2*Blockly.BlockSvg.SEP_SPACE_X;if(this.block_.previousConnection||this.block_.nextConnection)c.rightEdge=Math.max(c.rightEdge,Blockly.BlockSvg.NOTCH_WIDTH+Blockly.BlockSvg.SEP_SPACE_X);for(var d=0,e=0,f=!1,g=!1,h=!1,k=void 0,l=this.block_.inputsInline&&!this.block_.isCollapsed(),q=0,m;m=b[q];q++)if(m.isVisible()){var p;l&&k&&k!=Blockly.NEXT_STATEMENT&&m.type!=Blockly.NEXT_STATEMENT?p=c[c.length-1]:
(k=m.type,p=[],p.type=l&&m.type!=Blockly.NEXT_STATEMENT?Blockly.BlockSvg.INLINE:m.type,p.height=0,c.push(p));p.push(m);m.renderHeight=Blockly.BlockSvg.MIN_BLOCK_Y;m.renderWidth=l&&m.type==Blockly.INPUT_VALUE?Blockly.BlockSvg.TAB_WIDTH+Blockly.BlockSvg.SEP_SPACE_X:0;if(m.connection&&m.connection.targetConnection){var n=m.connection.targetBlock().getHeightWidth();m.renderHeight=Math.max(m.renderHeight,n.height);m.renderWidth=Math.max(m.renderWidth,n.width)}p.height=Math.max(p.height,m.renderHeight);
m.fieldWidth=0;1==c.length&&(m.fieldWidth+=Blockly.RTL?-a:a);for(var n=0,r;r=m.fieldRow[n];n++)0!=n&&(m.fieldWidth+=Blockly.BlockSvg.SEP_SPACE_X),r=r.getSize(),m.fieldWidth+=r.width,p.height=Math.max(p.height,r.height);p.type!=Blockly.BlockSvg.INLINE&&(p.type==Blockly.NEXT_STATEMENT?(g=!0,e=Math.max(e,m.fieldWidth)):(p.type==Blockly.INPUT_VALUE?f=!0:p.type==Blockly.DUMMY_INPUT&&(h=!0),d=Math.max(d,m.fieldWidth)))}for(a=0;p=c[a];a++)if(p.thicker=!1,p.type==Blockly.BlockSvg.INLINE)for(b=0;m=p[b];b++)if(m.type==
Blockly.INPUT_VALUE){p.height+=2*Blockly.BlockSvg.INLINE_PADDING_Y;p.thicker=!0;break}c.statementEdge=2*Blockly.BlockSvg.SEP_SPACE_X+e;g&&(c.rightEdge=Math.max(c.rightEdge,c.statementEdge+Blockly.BlockSvg.NOTCH_WIDTH));f?c.rightEdge=Math.max(c.rightEdge,d+2*Blockly.BlockSvg.SEP_SPACE_X+Blockly.BlockSvg.TAB_WIDTH):h&&(c.rightEdge=Math.max(c.rightEdge,d+2*Blockly.BlockSvg.SEP_SPACE_X));c.hasValue=f;c.hasStatement=g;c.hasDummy=h;return c};
...
n.connection.tighten_(),m==f.length-1||f[m+1].type==Blockly.NEXT_STATEMENT)&&(a.push("v",Blockly.BlockSvg.SEP_SPACE_Y),Blockly.RTL&&b.push("v",Blockly.BlockSvg.SEP_SPACE_Y-1),k+=Blockly.BlockSvg.SEP_SPACE_Y);k+=p.height}f.length||(k=Blockly.BlockSvg.MIN_BLOCK_Y,a.push("V",k),Blockly.RTL&&b.push("V",k-1));return k};
...
b.length;a++)c=b[a],c.targetConnection&&c.disconnect(),b[a].dispose();this.svg_&&(this.svg_.dispose(),this.svg_=null);Blockly.Realtime.isEnabled()&&!Blockly.Realtime.withinSync&&Blockly.Realtime.removeBlock(this)};
...
!0)}};c.push(d)}d={enabled:!(goog.isFunction(this.helpUrl)?!this.helpUrl():!this.helpUrl)};d.text=Blockly.Msg.HELP;d.callback=function(){b.showHelp_()};c.push(d);this.customContextMenu&&!b.isInFlyout&&this.customContextMenu(c);Blockly.ContextMenu.show(a,c)}};
...
// Copyright 2012 Google Inc.  Apache License 2.0
Blockly.FieldDropdown=function(a,b){this.menuGenerator_=a;this.changeHandler_=b;this.trimOptions_();var c=this.getOptions_()[0];this.value_=c[1];this.arrow_=Blockly.createSvgElement("tspan",{},null);this.arrow_.appendChild(document.createTextNode(Blockly.RTL?"\u25be ":" \u25be"));Blockly.FieldDropdown.superClass_.constructor.call(this,c[0])};goog.inherits(Blockly.FieldDropdown,Blockly.Field);Blockly.FieldDropdown.X_PADDING=20;Blockly.FieldDropdown.Y_HEIGHT=20;
...
l=0;l<k.length;l++){var q=k[l][1],m=Blockly.FieldDropdown.optionToDom_(k[l][0]),p=m.firstChild,n=m.lastChild;c.appendChild(m);h||q!=this.value_||(h=Blockly.createSvgElement("text",{"class":"blocklyMenuText",y:15},null),m.insertBefore(h,n),h.appendChild(document.createTextNode("\u2713")));m.setAttribute("transform","translate(0, "+l*Blockly.FieldDropdown.Y_HEIGHT+")");g.push(p);Blockly.bindEvent_(m,"mousedown",null,Blockly.noEvent);Blockly.bindEvent_(m,"mouseup",this,a(q));Blockly.bindEvent_(m,"mouseup",
null,Blockly.FieldDropdown.hide);f=Math.max(f,n.getComputedTextLength())}f+=2*Blockly.FieldDropdown.X_PADDING;for(l=0;l<g.length;l++)g[l].setAttribute("width",f);if(Blockly.RTL)for(l=0;m=c.childNodes[l];l++)n=m.lastChild,n.setAttribute("text-anchor","end"),n.setAttribute("x",f-Blockly.FieldDropdown.X_PADDING);h&&(Blockly.RTL?(h.setAttribute("text-anchor","end"),h.setAttribute("x",f-5)):h.setAttribute("x",5));l=f+2*Blockly.FieldDropdown.CORNER_RADIUS;k=k.length*Blockly.FieldDropdown.Y_HEIGHT+Blockly.FieldDropdown.CORNER_RADIUS+
1;e.setAttribute("width",l);e.setAttribute("height",k);d.setAttribute("width",l);d.setAttribute("height",k);e=Blockly.makeColour(this.sourceBlock_.getColour());d.setAttribute("fill",e);d=Blockly.getSvgXY_(this.borderRect_);e=this.borderRect_.getBBox();l=Blockly.RTL?d.x-f+Blockly.FieldDropdown.X_PADDING+e.width-Blockly.BlockSvg.SEP_SPACE_X/2:d.x-Blockly.FieldDropdown.X_PADDING+Blockly.BlockSvg.SEP_SPACE_X/2;b.setAttribute("transform","translate("+l+", "+(d.y+e.height)+")")};
...
this.sourceBlock_.rendered&&(this.sourceBlock_.render(),this.sourceBlock_.bumpNeighbours_(),this.sourceBlock_.workspace.fireChangeEvent()))};Blockly.FieldDropdown.optionToDom_=function(a){var b=Blockly.createSvgElement("g",{"class":"blocklyMenuDiv"},null);Blockly.createSvgElement("rect",{height:Blockly.FieldDropdown.Y_HEIGHT},b);var c=Blockly.createSvgElement("text",{"class":"blocklyMenuText",x:Blockly.FieldDropdown.X_PADDING,y:15},b);a=document.createTextNode(a);c.appendChild(a);return b};
Blockly.FieldDropdown.hide=function(){var a=Blockly.FieldDropdown.svgGroup_;a&&Blockly.addClass_(a,"blocklyHidden");Blockly.FieldDropdown.openDropdown_=null};
// Copyright 2012 Google Inc.  Apache License 2.0
...
Blockly.Generator.prototype.blockToCode=function(a){if(!a)return"";if(a.disabled)return a=a.nextConnection&&a.nextConnection.targetBlock(),this.blockToCode(a);var b=this[a.type];if(!b)throw'Language "'+this.name_+'" does not know how to generate code for block type "'+a.type+'".';b=b.call(a,a);return goog.isArray(b)?[this.scrub_(a,b[0]),b[1]]:this.scrub_(a,b)};
...
Blockly.Realtime.undoElementId_=null;Blockly.Realtime.redoElementId_=null;Blockly.Realtime.PROGRESS_URL_="media/progress.gif";Blockly.Realtime.initializeModel_=function(a){Blockly.Realtime.model_=a;var b=a.createMap();a.getRoot().set("blocks",b);b=a.createList();a.getRoot().set("topBlocks",b);Blockly.Realtime.chatBoxElementId_&&a.getRoot().set(Blockly.Realtime.chatBoxElementId_,a.createString(Blockly.Realtime.chatBoxInitialText_))};Blockly.Realtime.removeBlock=function(a){Blockly.Realtime.blocksMap_["delete"](a.id.toString())};
...
"relative";c.style.textAlign="center";c.style.border="1px solid";c.style.backgroundColor="#f6f9ff";c.style.borderRadius="15px";c.style.boxShadow="10px 10px 5px #888";c.style.width=b.width/3+"px";a=goog.style.getBounds(c);c.style.left=(b.width-a.width)/3+"px";c.style.top=(b.height-a.height)/4+"px";return c};Blockly.Realtime.doCommand=function(a){a()};
...
"}",".blocklyAngleCircle {","  stroke: #444;","  stroke-width: 1;","  fill: #ddd;","  fill-opacity: 0.8;","}",".blocklyAngleMarks {","  stroke: #444;","  stroke-width: 1;","}",".blocklyAngleGauge {","  fill: #f88;","  fill-opacity: 0.8;  ","}",".blocklyAngleLine {","  stroke: #f00;","  stroke-width: 2;","  stroke-linecap: round;","}","","/* Category tree in Toolbox. */",".blocklyToolboxDiv {","  background-color: #ddd;","  display: none;","  overflow-x: visible;","  overflow-y: auto;","  position: absolute;",
...
a.contentWidth>a.viewWidth+(Blockly.RTL?2:1)*a.viewLeft)for(var b=Blockly.mainWorkspace.getTopBlocks(!1),c=0,d;d=b[c];c++){var e=d.getRelativeToSurfaceXY(),q=d.getHeightWidth(),m=a.viewTop+25-q.height-e.y;0<m&&d.moveBy(0,m);m=a.viewTop+a.viewHeight-25-e.y;0>m&&d.moveBy(0,m);m=25+a.viewLeft-e.x-(Blockly.RTL?0:q.width);0<m&&d.moveBy(m,0);m=a.viewLeft+a.viewWidth-25-e.x+(Blockly.RTL?q.width:0);0>m&&d.moveBy(m,0);d.isDeletable()&&50<(Blockly.RTL?e.x-2*a.viewLeft-a.viewWidth:-e.x)&&d.dispose(!1,!0)}}})));
Blockly.Tooltip&&b.appendChild(Blockly.Tooltip.createDom());!Blockly.readOnly&&Blockly.FieldDropdown&&b.appendChild(Blockly.FieldDropdown.createDom());b.appendChild(Blockly.ContextMenu.createDom());a.appendChild(b);Blockly.svg=b;Blockly.svgResize();Blockly.WidgetDiv.DIV=goog.dom.createDom("div","blocklyWidgetDiv");document.body.appendChild(Blockly.WidgetDiv.DIV)};
...
Blockly.onMouseDown_=function(a){Blockly.svgResize();Blockly.terminateDrag_();Blockly.hideChaff();var b=a.target&&a.target.nodeName&&"svg"==a.target.nodeName.toLowerCase();!Blockly.readOnly&&Blockly.selected&&b&&Blockly.selected.unselect();Blockly.isRightButton(a)?Blockly.showContextMenu_(Blockly.mouseToSvg(a)):(Blockly.readOnly||b)&&Blockly.mainWorkspace.scrollbar&&(Blockly.mainWorkspace.dragMode=!0,Blockly.mainWorkspace.startDragMouseX=a.clientX,Blockly.mainWorkspace.startDragMouseY=a.clientY,Blockly.mainWorkspace.startDragMetrics=
Blockly.mainWorkspace.getMetrics(),Blockly.mainWorkspace.startScrollX=Blockly.mainWorkspace.scrollX,Blockly.mainWorkspace.startScrollY=Blockly.mainWorkspace.scrollY)};Blockly.onMouseUp_=function(a){Blockly.setCursorHand_(!1);Blockly.mainWorkspace.dragMode=!1};
...
b.push(c);Blockly.ContextMenu.show(a,b)}};Blockly.onContextMenu_=function(a){Blockly.isTargetInput_(a)||a.preventDefault()};Blockly.hideChaff=function(a){Blockly.Tooltip.hide();Blockly.ContextMenu.hide();Blockly.FieldDropdown.hide();Blockly.WidgetDiv.hide();!a&&Blockly.Toolbox.flyout_&&Blockly.Toolbox.flyout_.autoClose&&Blockly.Toolbox.clearSelection()};
...
Blockly.setCursorHand_=function(a){if(!Blockly.readOnly){var b="";a&&(b="url("+Blockly.pathToBlockly+"media/handclosed.cur) 7 3, auto");Blockly.selected&&(Blockly.selected.getSvgRoot().style.cursor=b);Blockly.svg.style.cursor=b}};
...
"use strict";function clearCanvas(){doc.el("content_XML").value="<xml></xml>",window.xml_dirty=!0,showBLL()}function showBLL(){hideOthers(),addclass(doc.el("btn_BLL"),"active"),doc.el("content_BLL").style.display="block";var e=doc.el("content_XML").value,l=null;if(Blockly.fireUiEvent(window,"resize"),window.xml_dirty){try{l=Blockly.Xml.textToDom(e)}catch(t){var o="ERROR PARSING XML\n\nSelect OK to abandon your changes.\nSelect Cancel to further edit the XML.",s=window.confirm(o.replace("%1",t));return s?showXML():void 0}Blockly.mainWorkspace.clear(),Blockly.Xml.domToWorkspace(Blockly.getMainWorkspace(),l),window.xml_dirty=!1}Blockly.fireUiEvent(window,"resize")}function showXML(){var e=Blockly.Xml.workspaceToDom(Blockly.mainWorkspace),l=Blockly.Xml.domToPrettyText(e),t=doc.el("content_XML");hideOthers(),addclass(doc.el("btn_XML"),"active"),t.style.display="block",t.value=l}function showHLL(){contentXMLToBLL(),hideOthers(),doc.el("content_HLL").style.display="block",addclass(doc.el("btn_HLL"),"active")}function showLLL(){showBLL();var e=Blockly.LLL.workspaceToCode(),l=doc.el("content_LLL");hideOthers(),addclass(doc.el("btn_LLL"),"active"),l.style.display="block",l.innerHTML=e}function showEVM(){hideOthers(),doc.el("content_EVM").style.display="block",addclass(doc.el("btn_EVM"),"active"),contentXMLToBLL()}function hideOthers(){doc.el("content_BLL").style.display="none",unclass(doc.el("btn_BLL"),"active"),doc.el("content_XML").style.display="none",unclass(doc.el("btn_XML"),"active"),doc.el("content_HLL").style.display="none",unclass(doc.el("btn_HLL"),"active"),doc.el("content_LLL").style.display="none",unclass(doc.el("btn_LLL"),"active"),doc.el("content_EVM").style.display="none",unclass(doc.el("btn_EVM"),"active")}goog.provide("Blockly.LLL"),goog.require("Blockly.Generator"),Blockly.LLL=new Blockly.Generator("LLL"),Blockly.LLL.addReservedWords("if,when,unless,for,and,or,seq,add,mul,sub,div,sdiv,mod,smod,exp,neg,lt,le,gt,ge,eq,not,myaddress,balance,txsender,txvalue,txdatan,txdata,blk_prevhash,blk_coinbase,blk_timestamp,blk_number,blk_difficulty,blk_nonce,basefee,stop,extro,mktx,suicide,sha256,ripemd160,ecmul,ecadd,ecsign,ecrecover,ecvalid,sha3,mload,mstore,sload,sstore,push,pop,dup,swap,jmp,jmpi,ind"),Blockly.LLL.ORDER_ATOMIC=0,Blockly.LLL.ORDER_MEMBER=1,Blockly.LLL.ORDER_NEW=1,Blockly.LLL.ORDER_FUNCTION_CALL=2,Blockly.LLL.ORDER_INCREMENT=3,Blockly.LLL.ORDER_DECREMENT=3,Blockly.LLL.ORDER_LOGICAL_NOT=4,Blockly.LLL.ORDER_BITWISE_NOT=4,Blockly.LLL.ORDER_UNARY_PLUS=4,Blockly.LLL.ORDER_UNARY_NEGATION=4,Blockly.LLL.ORDER_TYPEOF=4,Blockly.LLL.ORDER_VOID=4,Blockly.LLL.ORDER_DELETE=4,Blockly.LLL.ORDER_MULTIPLICATION=5,Blockly.LLL.ORDER_DIVISION=5,Blockly.LLL.ORDER_MODULUS=5,Blockly.LLL.ORDER_ADDITION=6,Blockly.LLL.ORDER_SUBTRACTION=6,Blockly.LLL.ORDER_BITWISE_SHIFT=7,Blockly.LLL.ORDER_RELATIONAL=8,Blockly.LLL.ORDER_IN=8,Blockly.LLL.ORDER_INSTANCEOF=8,Blockly.LLL.ORDER_EQUALITY=9,Blockly.LLL.ORDER_BITWISE_AND=10,Blockly.LLL.ORDER_BITWISE_XOR=11,Blockly.LLL.ORDER_BITWISE_OR=12,Blockly.LLL.ORDER_LOGICAL_AND=13,Blockly.LLL.ORDER_LOGICAL_OR=14,Blockly.LLL.ORDER_CONDITIONAL=15,Blockly.LLL.ORDER_ASSIGNMENT=16,Blockly.LLL.ORDER_COMMA=17,Blockly.LLL.ORDER_NONE=99,Blockly.LLL.INFINITE_LOOP_TRAP=null,Blockly.LLL.init=function(){if(Blockly.LLL.definitions_=Object.create(null),Blockly.LLL.functionNames_=Object.create(null),Blockly.Variables){Blockly.LLL.variableDB_?Blockly.LLL.variableDB_.reset():Blockly.LLL.variableDB_=new Blockly.Names(Blockly.LLL.RESERVED_WORDS_);for(var e=[],l=Blockly.Variables.allVariables(),t=0;t<l.length;t++)e[t]='("'+Blockly.LLL.variableDB_.getName(l[t],Blockly.Variables.NAME_TYPE)+'" 0)';Blockly.LLL.definitions_.variables=e.join("\n")}},Blockly.LLL.finish=function(e){var l,t=[];for(var o in Blockly.LLL.definitions_)t.push(Blockly.LLL.definitions_[o]);return l=t.join("\n\n")+"\n\n"+e,l=e},Blockly.LLL.scrubNakedValue=function(e){return e+"\n"},Blockly.LLL.quote_=function(e){return'"'+e+'"'},Blockly.LLL.scrub_=function(e,l){if(null===l)return"";var t="";if(!e.outputConnection||!e.outputConnection.targetConnection){var o=e.getCommentText();o&&(t+=this.prefixLines(o,";; ")+"\n");for(var s=0;s<e.inputList.length;s++)if(e.inputList[s].type==Blockly.INPUT_VALUE){var L=e.inputList[s].connection.targetBlock();if(L){var o=this.allNestedComments(L);o&&(t+=this.prefixLines(o,";; "))}}}var n=e.nextConnection&&e.nextConnection.targetBlock(),i=this.blockToCode(n);return t+l+i},goog.provide("Blockly.Blocks.LLL"),goog.require("Blockly.Blocks");var VALUE_COLOR=190,LIST_COLOR=260,FLOW_COLOR=210,LOOP_COLOR=160,MATH_COLOR=230,PROCEDURE_COLOR=290,TEXT_COLOR=160,STATEMENT_COLOR=330,COLOUR_COLOR=58;Blockly.Blocks.LLL_spend={init:function(){this.setColour(STATEMENT_COLOR),this.appendValueInput("MONEY").appendField("spend").setCheck("Number"),this.appendValueInput("TO").appendField("to"),this.setInputsInline(!0),this.setPreviousStatement(!0),this.setNextStatement(!0)}},Blockly.Blocks.LLL_blockinfo={init:function(){var e=[["prevhash","prevhash"],["coinbase","coinbase"],["timestamp","timestamp"],["number","number"],["difficulty","difficulty"],["total gas","gaslimit"]];this.setColour(VALUE_COLOR),this.setOutput(!0),this.appendDummyInput().appendField("block").appendField(new Blockly.FieldDropdown(e),"PROP")}},Blockly.Blocks.LLL_tx={init:function(){var e=[["value","callvalue"],["caller","caller"],["datasize","calldatasize"],["data","calldataload"],["gas left","gas"]];this.setColour(VALUE_COLOR),this.setOutput(!0),this.appendDummyInput().appendField("tx").appendField(new Blockly.FieldDropdown(e),"PROP")}},Blockly.Blocks.LLL_contract={init:function(){var e=[["address","address"],["balance","balance"]];this.setColour(VALUE_COLOR),this.setOutput(!0),this.appendDummyInput().appendField("contract").appendField(new Blockly.FieldDropdown(e),"PROP")}},Blockly.Blocks.LLL_hash={init:function(){this.setColour(MATH_COLOR),this.setOutput(!0),this.appendDummyInput().appendField("ID for data"),this.appendValueInput("DATA_START").appendField("from start slot"),this.appendValueInput("DATA_END").appendField("thru end slot"),this.setInputsInline(!1)}},Blockly.Blocks.LLL_return={init:function(){this.setColour(STATEMENT_COLOR),this.appendDummyInput().appendField("reply with data"),this.appendValueInput("DATA_START").appendField("from start slot"),this.appendValueInput("DATA_END").appendField("thru end slot"),this.setPreviousStatement(!0),this.setInputsInline(!1)}},Blockly.Blocks.LLL_singleop={init:function(){var e=[["not","not"],["negative","neg"]];this.setColour(MATH_COLOR),this.setOutput(!0),this.appendValueInput("A").setCheck("Number").appendField(new Blockly.FieldDropdown(e),"OP"),this.setInputsInline(!0)}},Blockly.Blocks.LLL_forloop={init:function(){this.setColour(LOOP_COLOR),this.appendStatementInput("FIRST").appendField("one time do"),this.appendValueInput("COND").setCheck("Boolean").appendField("then as long as"),this.appendStatementInput("LOOP").appendField("repeatedly do"),this.appendStatementInput("AFTER_EACH").appendField("& each loop do"),this.setPreviousStatement(!0),this.setNextStatement(!0)}},Blockly.Blocks.LLL_whileloop={init:function(){var e=[["while","WHILE"],["until","UNTIL"]];this.setColour(LOOP_COLOR),this.appendValueInput("COND").setCheck("Boolean").appendField(new Blockly.FieldDropdown(e),"WORD"),this.appendStatementInput("DO").appendField("repeat"),this.setPreviousStatement(!0),this.setNextStatement(!0)}},Blockly.Blocks.LLL_call={init:function(){this.setColour(PROCEDURE_COLOR),this.appendValueInput("ADDRESS").appendField("call address"),this.appendValueInput("MONEY").appendField("   sending amount"),this.appendValueInput("GAS").appendField("   with fee budget"),this.appendDummyInput().appendField("and data from"),this.appendValueInput("SEND_DATA_START").appendField("   start slot"),this.appendValueInput("SEND_DATA_END").appendField("   thru end slot"),this.appendDummyInput().appendField("getting reply into"),this.appendValueInput("REPLY_DATA_START").appendField("   start slot"),this.appendValueInput("REPLY_DATA_BYTES").appendField("   with byte length"),this.setInputsInline(!1),this.setPreviousStatement(!0),this.setNextStatement(!0)}},Blockly.Blocks.LLL_init={init:function(){this.setColour(LOOP_COLOR),this.appendDummyInput().appendField("contract"),this.appendStatementInput("INIT").appendField("at birth"),this.appendStatementInput("BODY").appendField("on call"),this.setInputsInline(!1),this.setPreviousStatement(!1),this.setNextStatement(!1)}},Blockly.Blocks.LLL_val={init:function(){var e=function(e){var l=e?e:"",t="0X"==(l+"").substr(0,2).toUpperCase(),o=null===/[^0-9A-FX]/i.exec(l);if(t&&o)return String(e);if(isNaN(l)){var s=l.replace(/,/g,"");isNaN(s)||(l=s)}return l&&!isNaN(l)?(l=parseInt("0"+l),l=Math.abs(l)):l&&isNaN(l)&&(l=l.substr(0,32)),String(l)};this.setColour(VALUE_COLOR),this.appendDummyInput().appendField(new Blockly.FieldTextInput("",e),"VAL"),this.setOutput(!0)}},Blockly.Blocks.LLL_block={init:function(){var e=[["timestamp","timestamp"],["number","number"],["basefee","basefee"],["prevhash","prevhash"],["coinbase","coinbase"],["difficulty","difficulty"],["nonce","nonce"]];this.setColour(VALUE_COLOR),this.setOutput(!0),this.appendDummyInput().appendField("block").appendField(new Blockly.FieldDropdown(e),"PROP")}},Blockly.Blocks.LLL_transaction={init:function(){var e=[["value","value"],["sender","sender"],["data count","datan"]];this.setColour(VALUE_COLOR),this.setOutput(!0),this.appendDummyInput().appendField("tx").appendField(new Blockly.FieldDropdown(e),"PROP")}},Blockly.Blocks.LLL_contract={init:function(){var e=[["balance","balance"],["address","address"]];this.setColour(VALUE_COLOR),this.setOutput(!0),this.appendDummyInput().appendField("contract").appendField(new Blockly.FieldDropdown(e),"PROP")}},Blockly.Blocks.LLL_currency={init:function(){var e=[["wei","wei"],["Kwei","Kwei"],["Mwei","Mwei"],["Gwei","Gwei"],["szabo","szabo"],["finney","finney"],["ether","ether"],["Kether","Kether"],["Mether","Mether"],["Gether","Gether"],["Tether","Tether"],["Pether","Pether"],["Eether","Eether"],["Zether","Zether"],["Yether","Yether"],["Nether","Nether"],["Dether","Dether"],["Vether","Vether"],["Uether","Uether"]];this.setColour(VALUE_COLOR),this.appendValueInput("AMT").setCheck("Number"),this.appendDummyInput().appendField(new Blockly.FieldDropdown(e),"DENOM"),this.setInputsInline(!0),this.setOutput(!0)}},Blockly.Blocks.LLL_if={init:function(){this.setColour(FLOW_COLOR),this.appendValueInput("COND").setCheck("Boolean").appendField("if"),this.appendStatementInput("THEN").appendField("then"),this.appendStatementInput("ELSE").appendField("else"),this.setPreviousStatement(!0),this.setNextStatement(!0)}},Blockly.Blocks.LLL_when={init:function(){var e=[["when","when"],["unless","unless"]];this.setColour(FLOW_COLOR),this.appendValueInput("COND").setCheck("Boolean").appendField(new Blockly.FieldDropdown(e),"WORD"),this.appendStatementInput("THEN").appendField("then"),this.setPreviousStatement(!0),this.setNextStatement(!0)}},Blockly.Blocks.LLL_for={init:function(){var e=[["while","WHILE"],["until","UNTIL"]];this.setColour(LOOP_COLOR),this.appendValueInput("COND").setCheck("Boolean").appendField("repeat").appendField(new Blockly.FieldDropdown(e),"WORD"),this.appendStatementInput("DO").appendField("do"),this.setPreviousStatement(!0),this.setNextStatement(!0)}},Blockly.Blocks.LLL_compare={init:function(){var e=[[">",">"],["<","<"],["=","="],["≠","!="],["≤","<="],["≥",">="]];this.setColour(MATH_COLOR),this.setOutput(!0),this.appendValueInput("A").setCheck("Number"),this.appendValueInput("B").setCheck("Number").appendField(new Blockly.FieldDropdown(e),"OP"),this.setInputsInline(!0)}},Blockly.Blocks.LLL_logic={init:function(){var e=[["and","and"],["or","or"]];this.setColour(MATH_COLOR),this.setOutput(!0),this.appendValueInput("A").setCheck("Number"),this.appendValueInput("B").setCheck("Number").appendField(new Blockly.FieldDropdown(e),"OP"),this.setInputsInline(!1)}},Blockly.Blocks.LLL_math={init:function(){var e=[["+","add"],["×","mul"],["-","sub"],["÷","sdiv"],["raised to","exp"],["modulo","smod"]];this.setColour(MATH_COLOR),this.setOutput(!0),this.appendValueInput("A").setCheck("Number"),this.appendValueInput("B").setCheck("Number").appendField(new Blockly.FieldDropdown(e),"OP"),this.setInputsInline(!0)}},Blockly.Blocks.LLL_load={init:function(){var e=[["storage","sload"],["memory","mload"],["tx data","calldataload"]];this.setColour(LIST_COLOR),this.appendDummyInput().appendField(new Blockly.FieldDropdown(e),"PLACE"),this.appendValueInput("SLOT").setCheck("Number").appendField("slot"),this.appendDummyInput().appendField("content"),this.setOutput(!0),this.setInputsInline(!0)}},Blockly.Blocks.LLL_balance={init:function(){this.setColour(MATH_COLOR),this.appendValueInput("ADDR").appendField("balance of"),this.setInputsInline(!0),this.setOutput(!0)}},Blockly.Blocks.LLL_comment={init:function(){this.setColour(COLOUR_COLOR),this.appendDummyInput().appendField("note:").appendField(new Blockly.FieldTextInput(""),"NOTE"),this.setPreviousStatement(!0),this.setNextStatement(!0)}},Blockly.Blocks.LLL_stop={init:function(){this.setColour(STATEMENT_COLOR),this.appendDummyInput().appendField("stop"),this.setPreviousStatement(!0)}},Blockly.Blocks.LLL_suicide={init:function(){this.setColour(STATEMENT_COLOR),this.appendValueInput("TO").appendField("self-destruct to"),this.setPreviousStatement(!0),this.setInputsInline(!0),this.setTooltip("contract sends full balance to specified address, then self-destructs")}},Blockly.Blocks.LLL_mktx={init:function(){this.setColour(STATEMENT_COLOR),this.appendValueInput("MONEY").appendField("spend").setCheck("Number"),this.appendValueInput("TO").appendField("to"),this.setInputsInline(!0),this.setPreviousStatement(!0),this.setNextStatement(!0)}},Blockly.Blocks.LLL_store={init:function(){var e=[["storage","sstore"],["memory","mstore"]];this.setColour(LIST_COLOR),this.appendValueInput("VAL").appendField("put"),this.appendValueInput("SLOT").appendField("in").appendField(new Blockly.FieldDropdown(e),"PLACE").appendField("slot").setCheck("Number"),this.setInputsInline(!0),this.setPreviousStatement(!0),this.setNextStatement(!0)}},goog.provide("Blockly.LLL.LLLblocks_generator"),goog.require("Blockly.LLL"),Blockly.LLL.LLL_spend=function(e){var l=Blockly.LLL.ORDER_NONE,t=Blockly.LLL.valueToCode(e,"TO",l)||0,o=Blockly.LLL.valueToCode(e,"MONEY",l)||"0wei",s="call",L="0",n="0",i="0",_="0",T="0",c="(call"+t+" "+o+" 0)\n",c="("+s+" "+t+" "+o+" "+L+" "+n+" "+i+" "+_+" "+T+")\n";return c},Blockly.LLL.LLL_singleop=function(e){var l=e.getFieldValue("OP"),t=Blockly.LLL.valueToCode(e,"A",Blockly.LLL.ORDER_NONE),o="("+l+" "+t+")";return[o,Blockly.LLL.ORDER_ATOMIC]},Blockly.LLL.LLL_hash=function(e){var l="sha3",t=Blockly.LLL.valueToCode(e,"DATA_START",Blockly.LLL.ORDER_NONE),o=Blockly.LLL.valueToCode(e,"DATA_END",Blockly.LLL.ORDER_NONE),s="("+l+" "+t+" "+o+")";return[s,Blockly.LLL.ORDER_ATOMIC]},Blockly.LLL.LLL_call=function(e){var l="call",t=Blockly.LLL.valueToCode(e,"ADDRESS",Blockly.LLL.ORDER_NONE),o=Blockly.LLL.valueToCode(e,"MONEY",Blockly.LLL.ORDER_NONE),s=Blockly.LLL.valueToCode(e,"GAS",Blockly.LLL.ORDER_NONE),L=Blockly.LLL.valueToCode(e,"SEND_DATA_START",Blockly.LLL.ORDER_NONE),n=Blockly.LLL.valueToCode(e,"SEND_DATA_END",Blockly.LLL.ORDER_NONE),i=Blockly.LLL.valueToCode(e,"REPLY_DATA_START",Blockly.LLL.ORDER_NONE),_=Blockly.LLL.valueToCode(e,"REPLY_DATA_BYTES",Blockly.LLL.ORDER_NONE),T=32*(n-L),c="("+l+" "+t+" "+o+" "+s+" "+L+" "+T+" "+i+" "+_+")\n";return c},Blockly.LLL.LLL_return=function(e){var l="return",t=Blockly.LLL.valueToCode(e,"DATA_START",Blockly.LLL.ORDER_NONE),o=Blockly.LLL.valueToCode(e,"DATA_END",Blockly.LLL.ORDER_NONE),s="("+l+" "+t+" "+o+")\n";return s},Blockly.LLL.LLL_blockinfo=function(e){var l,t=e.getFieldValue("PROP");return l="("+t+")",[l,Blockly.LLL.ORDER_ATOMIC]},Blockly.LLL.LLL_tx=function(e){var l,t=e.getFieldValue("PROP");return l="("+t+")",[l,Blockly.LLL.ORDER_ATOMIC]},Blockly.LLL.LLL_contract=function(e){var l,t=e.getFieldValue("PROP");return l="balance"==t?"(balance (address))":"("+t+")",[l,Blockly.LLL.ORDER_ATOMIC]},Blockly.LLL.LLL_forloop=function(e){var l=Blockly.LLL.valueToCode(e,"COND",Blockly.LLL.ORDER_NONE)||"false",t=Blockly.LLL.statementToCode(e,"FIRST"),o=Blockly.LLL.statementToCode(e,"AFTER_EACH"),s=Blockly.LLL.statementToCode(e,"LOOP");return"(for\n"+t+"  "+l+"\n"+o+"  (seq \n"+s+"  )  \n)\n"},Blockly.LLL.LLL_init=function(e){var l=Blockly.LLL.statementToCode(e,"INIT"),t=Blockly.LLL.statementToCode(e,"BODY");return"(seq\n\n"+l+"\n)\n\n(seq\n\n"+t+"\n)"},Blockly.LLL.LLL_whileloop=function(e){var l="UNTIL"==e.getFieldValue("WORD"),t=Blockly.LLL.valueToCode(e,"COND",l?Blockly.LLL.ORDER_LOGICAL_NOT:Blockly.LLL.ORDER_NONE)||"false",o=Blockly.LLL.statementToCode(e,"DO");return l&&(t="(not "+t+" )"),"(for () "+t+"\n (seq \n"+o+" )\n)\n"},Blockly.LLL.LLL_if=function(e){var l=Blockly.LLL.valueToCode(e,"COND",Blockly.LLL.ORDER_NONE),t=Blockly.LLL.statementToCode(e,"THEN"),o=Blockly.LLL.statementToCode(e,"ELSE"),s="(if "+l+"\n (seq \n"+t+" )\n (seq \n"+o+" )\n)\n";return s},Blockly.LLL.LLL_when=function(e){var l=e.getFieldValue("WORD"),t=Blockly.LLL.valueToCode(e,"COND",Blockly.LLL.ORDER_NONE),o=Blockly.LLL.statementToCode(e,"THEN"),s="("+l+" "+t+"\n (seq \n"+o+" )\n)\n";return s},Blockly.LLL.twoArgForms=function(e){var l=e.getFieldValue("OP"),t=Blockly.LLL.valueToCode(e,"A",Blockly.LLL.ORDER_NONE),o=Blockly.LLL.valueToCode(e,"B",Blockly.LLL.ORDER_NONE),s="("+l+" "+t+" "+o+")";return[s,Blockly.LLL.ORDER_ATOMIC]},Blockly.LLL.LLL_math=Blockly.LLL.twoArgForms,Blockly.LLL.LLL_logic=Blockly.LLL.twoArgForms,Blockly.LLL.LLL_compare=Blockly.LLL.twoArgForms,Blockly.LLL.LLL_val=function(e){var l=(Blockly.LLL.ORDER_NONE,e.getFieldValue("VAL")||0),t="";if(isNaN(l)){var o="0X"==(l+"").substr(0,2).toUpperCase(),s=null===/[^0-9a-fx]/i.exec(l);t=o&&s?l:'"'+l+'"'}else t=0>l?"(neg "+-l+")":l+"";return[t,Blockly.LLL.ORDER_ATOMIC]},Blockly.LLL.LLL_comment=function(e){return";; "+e.getFieldValue("NOTE")+"\n"},Blockly.LLL.LLL_stop=function(){return"(stop)\n"},Blockly.LLL.LLL_currency=function(e){var l=Blockly.LLL.ORDER_NONE,t=Blockly.LLL.valueToCode(e,"AMT",l)||0,o=e.getFieldValue("DENOM"),s=t+""+o;return[s,Blockly.LLL.ORDER_ATOMIC]},Blockly.LLL.LLL_suicide=function(e){var l=Blockly.LLL.ORDER_NONE,t=Blockly.LLL.valueToCode(e,"TO",l)||0,o="(suicide "+t+")\n";return o},Blockly.LLL.LLL_load=function(e){var l=Blockly.LLL.ORDER_NONE,t=e.getFieldValue("PLACE"),o=Blockly.LLL.valueToCode(e,"SLOT",l)||0,s="("+t+" "+o+")";return[s,Blockly.LLL.ORDER_ATOMIC]},Blockly.LLL.LLL_store=function(e){var l=Blockly.LLL.ORDER_NONE,t=e.getFieldValue("PLACE"),o=Blockly.LLL.valueToCode(e,"SLOT",l)||0,s=Blockly.LLL.valueToCode(e,"VAL",l)||0,L="("+t+" "+o+" "+s+")\n";return L},Blockly.LLL.LLL_balance=function(e){var l=(Blockly.LLL.ORDER_NONE,e.getFieldValue("ADDR")),t="(balance "+l+")";return[t,Blockly.LLL.ORDER_ATOMIC]},Blockly.LLL.LLL_block=function(e){var l,t=e.getFieldValue("PROP");return l="basefee"==t?"(basefee)":"(blk_"+t+")",[l,Blockly.LLL.ORDER_ATOMIC]},Blockly.LLL.LLL_transaction=function(e){var l,t=e.getFieldValue("PROP");return l="(tx"+t+")",[l,Blockly.LLL.ORDER_ATOMIC]},Blockly.LLL.LLL_contract=function(e){var l,t=e.getFieldValue("PROP");return"address"==t?l="(myaddress)":"balance"==t&&(l="(balance myaddress)"),[l,Blockly.LLL.ORDER_ATOMIC]},Blockly.LLL.LLL_for=function(e){var l="UNTIL"==e.getFieldValue("WORD"),t=Blockly.LLL.valueToCode(e,"COND",l?Blockly.LLL.ORDER_LOGICAL_NOT:Blockly.LLL.ORDER_NONE)||"false",o=Blockly.LLL.statementToCode(e,"DO");return l&&(t="(! "+t+" )"),"(for "+t+"\n (seq \n"+o+" ) \n)\n"},Blockly.LLL.LLL_mktx=function(e){var l=Blockly.LLL.ORDER_NONE,t=Blockly.LLL.valueToCode(e,"TO",l)||0,o=Blockly.LLL.valueToCode(e,"MONEY",l)||"0wei",s="(mktx "+t+" "+o+" 0)\n";return s};var BlocklyStorage={};BlocklyStorage.backupBlocks_=function(){if("localStorage"in window){var e=Blockly.Xml.workspaceToDom(Blockly.getMainWorkspace()),l=window.location.href.split("#")[0];window.localStorage.setItem(l,Blockly.Xml.domToText(e))}},BlocklyStorage.backupOnUnload=function(){window.addEventListener("unload",BlocklyStorage.backupBlocks_,!1)},BlocklyStorage.restoreBlocks=function(){var e=window.location.href.split("#")[0];if("localStorage"in window&&window.localStorage[e]){var l=Blockly.Xml.textToDom(window.localStorage[e]);Blockly.Xml.domToWorkspace(Blockly.getMainWorkspace(),l)}},BlocklyStorage.link=function(){var e=Blockly.Xml.workspaceToDom(Blockly.getMainWorkspace()),l=Blockly.Xml.domToText(e);BlocklyStorage.makeRequest_("/storage","xml",l)},BlocklyStorage.retrieveXml=function(e){Blockly.Xml.workspaceToDom(Blockly.getMainWorkspace());BlocklyStorage.makeRequest_("/storage","key",e)},BlocklyStorage.httpRequest_=null,BlocklyStorage.makeRequest_=function(e,l,t){BlocklyStorage.httpRequest_&&BlocklyStorage.httpRequest_.abort(),BlocklyStorage.httpRequest_=new XMLHttpRequest,BlocklyStorage.httpRequest_.name=l,BlocklyStorage.httpRequest_.onreadystatechange=BlocklyStorage.handleRequest_,BlocklyStorage.httpRequest_.open("POST",e),BlocklyStorage.httpRequest_.setRequestHeader("Content-Type","application/x-www-form-urlencoded"),BlocklyStorage.httpRequest_.send(l+"="+encodeURIComponent(t))},BlocklyStorage.handleRequest_=function(){if(4==BlocklyStorage.httpRequest_.readyState){if(200!=BlocklyStorage.httpRequest_.status)BlocklyStorage.alert(BlocklyStorage.HTTPREQUEST_ERROR+"\nhttpRequest_.status: "+BlocklyStorage.httpRequest_.status);else{var e=BlocklyStorage.httpRequest_.responseText.trim();"xml"==BlocklyStorage.httpRequest_.name?(window.location.hash=e,BlocklyStorage.alert(BlocklyStorage.LINK_ALERT.replace("%1",window.location.href))):"key"==BlocklyStorage.httpRequest_.name&&(e.length?BlocklyStorage.loadXml_(e):BlocklyStorage.alert(BlocklyStorage.HASH_ERROR.replace("%1",window.location.hash))),BlocklyStorage.monitorChanges_()}BlocklyStorage.httpRequest_=null}},BlocklyStorage.monitorChanges_=function(){function e(){var e=Blockly.Xml.workspaceToDom(Blockly.getMainWorkspace()),l=Blockly.Xml.domToText(e);t!=l&&(window.location.hash="",Blockly.removeChangeListener(o))}var l=Blockly.Xml.workspaceToDom(Blockly.getMainWorkspace()),t=Blockly.Xml.domToText(l),o=Blockly.addChangeListener(e)},BlocklyStorage.loadXml_=function(e){try{e=Blockly.Xml.textToDom(e)}catch(l){return void BlocklyStorage.alert(BlocklyStorage.XML_ERROR+"\nXML: "+e)}Blockly.getMainWorkspace().clear(),Blockly.Xml.domToWorkspace(Blockly.getMainWorkspace(),e)},BlocklyStorage.alert=function(e){window.alert(e)},goog.provide("Blockly.Msg.en"),goog.require("Blockly.Msg"),Blockly.Msg.VARIABLES_DEFAULT_NAME="item",Blockly.Msg.DUPLICATE_BLOCK="Duplicate",Blockly.Msg.ADD_COMMENT="Add Comment",Blockly.Msg.REMOVE_COMMENT="Remove Comment",Blockly.Msg.EXTERNAL_INPUTS="Show Multiple Lines",Blockly.Msg.INLINE_INPUTS="Show Single Line",Blockly.Msg.DELETE_BLOCK="Delete Block",Blockly.Msg.DELETE_X_BLOCKS="Delete %1 Blocks",Blockly.Msg.COLLAPSE_BLOCK="Collapse Block",Blockly.Msg.COLLAPSE_ALL="Collapse Blocks",Blockly.Msg.EXPAND_BLOCK="Expand Block",Blockly.Msg.EXPAND_ALL="Expand Blocks",Blockly.Msg.DISABLE_BLOCK="Disable Block",Blockly.Msg.ENABLE_BLOCK="Enable Block",Blockly.Msg.HELP="Help",Blockly.Msg.CHAT="Chat with your collaborator by typing in this box!",Blockly.Msg.AUTH="Please authorize this app to enable your work to be saved and to allow it to be shared by you.",Blockly.Msg.CHANGE_VALUE_TITLE="Change value:",Blockly.Msg.NEW_VARIABLE="New variable...",Blockly.Msg.NEW_VARIABLE_TITLE="New variable name:",Blockly.Msg.RENAME_VARIABLE="Rename variable...",Blockly.Msg.RENAME_VARIABLE_TITLE='Rename all "%1" variables to:',Blockly.Msg.COLOUR_PICKER_HELPURL="https://en.wikipedia.org/wiki/Color",Blockly.Msg.COLOUR_PICKER_TOOLTIP="Choose a colour from the palette.",Blockly.Msg.COLOUR_RANDOM_HELPURL="http://randomcolour.com",Blockly.Msg.COLOUR_RANDOM_TITLE="random colour",Blockly.Msg.COLOUR_RANDOM_TOOLTIP="Choose a colour at random.",Blockly.Msg.COLOUR_RGB_HELPURL="http://www.december.com/html/spec/colorper.html",Blockly.Msg.COLOUR_RGB_TITLE="colour with",Blockly.Msg.COLOUR_RGB_RED="red",Blockly.Msg.COLOUR_RGB_GREEN="green",Blockly.Msg.COLOUR_RGB_BLUE="blue",Blockly.Msg.COLOUR_RGB_TOOLTIP="Create a colour with the specified amount of red, green, and blue.  All values must be between 0 and 100.",Blockly.Msg.COLOUR_BLEND_HELPURL="http://meyerweb.com/eric/tools/color-blend/",Blockly.Msg.COLOUR_BLEND_TITLE="blend",Blockly.Msg.COLOUR_BLEND_COLOUR1="colour 1",Blockly.Msg.COLOUR_BLEND_COLOUR2="colour 2",Blockly.Msg.COLOUR_BLEND_RATIO="ratio",Blockly.Msg.COLOUR_BLEND_TOOLTIP="Blends two colours together with a given ratio (0.0 - 1.0).",Blockly.Msg.CONTROLS_REPEAT_HELPURL="https://en.wikipedia.org/wiki/For_loop",Blockly.Msg.CONTROLS_REPEAT_TITLE="repeat %1 times",Blockly.Msg.CONTROLS_REPEAT_TITLE_REPEAT="repeat",Blockly.Msg.CONTROLS_REPEAT_TITLE_TIMES="times",Blockly.Msg.CONTROLS_REPEAT_INPUT_DO="do",Blockly.Msg.CONTROLS_REPEAT_TOOLTIP="Do some statements several times.",Blockly.Msg.CONTROLS_WHILEUNTIL_HELPURL="https://code.google.com/p/blockly/wiki/Repeat",Blockly.Msg.CONTROLS_WHILEUNTIL_INPUT_DO=Blockly.Msg.CONTROLS_REPEAT_INPUT_DO,Blockly.Msg.CONTROLS_WHILEUNTIL_OPERATOR_WHILE="repeat while",Blockly.Msg.CONTROLS_WHILEUNTIL_OPERATOR_UNTIL="repeat until",Blockly.Msg.CONTROLS_WHILEUNTIL_TOOLTIP_WHILE="While a value is true, then do some statements.",Blockly.Msg.CONTROLS_WHILEUNTIL_TOOLTIP_UNTIL="While a value is false, then do some statements.",Blockly.Msg.CONTROLS_FOR_HELPURL="https://code.google.com/p/blockly/wiki/Loops#count_with",Blockly.Msg.CONTROLS_FOR_TOOLTIP="Have the variable %1 take on the values from the start number to the end number, counting by the specified interval, and do the specified blocks.",Blockly.Msg.CONTROLS_FOR_INPUT_WITH="count with",Blockly.Msg.CONTROLS_FOR_INPUT_FROM_TO_BY="from %1 to %2 by %3",Blockly.Msg.CONTROLS_FOR_INPUT_DO=Blockly.Msg.CONTROLS_REPEAT_INPUT_DO,Blockly.Msg.CONTROLS_FOREACH_HELPURL="https://code.google.com/p/blockly/wiki/Loops#for_each for each block",Blockly.Msg.CONTROLS_FOREACH_INPUT_ITEM="for each item",Blockly.Msg.CONTROLS_FOREACH_INPUT_INLIST="in list",Blockly.Msg.CONTROLS_FOREACH_INPUT_INLIST_TAIL="",Blockly.Msg.CONTROLS_FOREACH_INPUT_DO=Blockly.Msg.CONTROLS_REPEAT_INPUT_DO,Blockly.Msg.CONTROLS_FOREACH_TOOLTIP='For each item in a list, set the variable "%1" to the item, and then do some statements.',Blockly.Msg.CONTROLS_FLOW_STATEMENTS_HELPURL="https://code.google.com/p/blockly/wiki/Loops#Loop_Termination_Blocks",Blockly.Msg.CONTROLS_FLOW_STATEMENTS_OPERATOR_BREAK="break out of loop",Blockly.Msg.CONTROLS_FLOW_STATEMENTS_OPERATOR_CONTINUE="continue with next iteration of loop",Blockly.Msg.CONTROLS_FLOW_STATEMENTS_TOOLTIP_BREAK="Break out of the containing loop.",Blockly.Msg.CONTROLS_FLOW_STATEMENTS_TOOLTIP_CONTINUE="Skip the rest of this loop, and continue with the next iteration.",Blockly.Msg.CONTROLS_FLOW_STATEMENTS_WARNING="Warning: This block may only be used within a loop.",Blockly.Msg.CONTROLS_IF_HELPURL="https://code.google.com/p/blockly/wiki/If_Then",Blockly.Msg.CONTROLS_IF_TOOLTIP_1="If a value is true, then do some statements.",Blockly.Msg.CONTROLS_IF_TOOLTIP_2="If a value is true, then do the first block of statements.  Otherwise, do the second block of statements.",Blockly.Msg.CONTROLS_IF_TOOLTIP_3="If the first value is true, then do the first block of statements.  Otherwise, if the second value is true, do the second block of statements.",Blockly.Msg.CONTROLS_IF_TOOLTIP_4="If the first value is true, then do the first block of statements.  Otherwise, if the second value is true, do the second block of statements.  If none of the values are true, do the last block of statements.",Blockly.Msg.CONTROLS_IF_MSG_IF="if",Blockly.Msg.CONTROLS_IF_MSG_ELSEIF="else if",Blockly.Msg.CONTROLS_IF_MSG_ELSE="else",Blockly.Msg.CONTROLS_IF_MSG_THEN=Blockly.Msg.CONTROLS_REPEAT_INPUT_DO,Blockly.Msg.CONTROLS_IF_IF_TITLE_IF=Blockly.Msg.CONTROLS_IF_MSG_IF,Blockly.Msg.CONTROLS_IF_IF_TOOLTIP="Add, remove, or reorder sections to reconfigure this if block.",Blockly.Msg.CONTROLS_IF_ELSEIF_TITLE_ELSEIF=Blockly.Msg.CONTROLS_IF_MSG_ELSEIF,Blockly.Msg.CONTROLS_IF_ELSEIF_TOOLTIP="Add a condition to the if block.",Blockly.Msg.CONTROLS_IF_ELSE_TITLE_ELSE=Blockly.Msg.CONTROLS_IF_MSG_ELSE,Blockly.Msg.CONTROLS_IF_ELSE_TOOLTIP="Add a final, catch-all condition to the if block.",Blockly.Msg.LOGIC_COMPARE_HELPURL="https://en.wikipedia.org/wiki/Inequality_(mathematics)",Blockly.Msg.LOGIC_COMPARE_TOOLTIP_EQ="Return true if both inputs equal each other.",Blockly.Msg.LOGIC_COMPARE_TOOLTIP_NEQ="Return true if both inputs are not equal to each other.",Blockly.Msg.LOGIC_COMPARE_TOOLTIP_LT="Return true if the first input is smaller than the second input.",Blockly.Msg.LOGIC_COMPARE_TOOLTIP_LTE="Return true if the first input is smaller than or equal to the second input.",Blockly.Msg.LOGIC_COMPARE_TOOLTIP_GT="Return true if the first input is greater than the second input.",Blockly.Msg.LOGIC_COMPARE_TOOLTIP_GTE="Return true if the first input is greater than or equal to the second input.",Blockly.Msg.LOGIC_OPERATION_HELPURL="https://code.google.com/p/blockly/wiki/And_Or",Blockly.Msg.LOGIC_OPERATION_TOOLTIP_AND="Return true if both inputs are true.",Blockly.Msg.LOGIC_OPERATION_AND="and",Blockly.Msg.LOGIC_OPERATION_TOOLTIP_OR="Return true if at least one of the inputs is true.",Blockly.Msg.LOGIC_OPERATION_OR="or",Blockly.Msg.LOGIC_NEGATE_HELPURL="https://code.google.com/p/blockly/wiki/Not",Blockly.Msg.LOGIC_NEGATE_TITLE="not %1",Blockly.Msg.LOGIC_NEGATE_TOOLTIP="Returns true if the input is false.  Returns false if the input is true.",Blockly.Msg.LOGIC_BOOLEAN_HELPURL="https://code.google.com/p/blockly/wiki/True_False",Blockly.Msg.LOGIC_BOOLEAN_TRUE="true",Blockly.Msg.LOGIC_BOOLEAN_FALSE="false",Blockly.Msg.LOGIC_BOOLEAN_TOOLTIP="Returns either true or false.",Blockly.Msg.LOGIC_NULL_HELPURL="https://en.wikipedia.org/wiki/Nullable_type",Blockly.Msg.LOGIC_NULL="null",Blockly.Msg.LOGIC_NULL_TOOLTIP="Returns null.",Blockly.Msg.LOGIC_TERNARY_HELPURL="https://en.wikipedia.org/wiki/%3F:",Blockly.Msg.LOGIC_TERNARY_CONDITION="test",Blockly.Msg.LOGIC_TERNARY_IF_TRUE="if true",Blockly.Msg.LOGIC_TERNARY_IF_FALSE="if false",Blockly.Msg.LOGIC_TERNARY_TOOLTIP='Check the condition in "test". If the condition is true, returns the "if true" value; otherwise returns the "if false" value.',Blockly.Msg.MATH_NUMBER_HELPURL="https://en.wikipedia.org/wiki/Number",Blockly.Msg.MATH_NUMBER_TOOLTIP="A number.",Blockly.Msg.MATH_ADDITION_SYMBOL="+",Blockly.Msg.MATH_SUBTRACTION_SYMBOL="-",Blockly.Msg.MATH_DIVISION_SYMBOL="÷",Blockly.Msg.MATH_MULTIPLICATION_SYMBOL="×",Blockly.Msg.MATH_POWER_SYMBOL="^",Blockly.Msg.MATH_TRIG_SIN="sin",Blockly.Msg.MATH_TRIG_COS="cos",Blockly.Msg.MATH_TRIG_TAN="tan",Blockly.Msg.MATH_TRIG_ASIN="asin",Blockly.Msg.MATH_TRIG_ACOS="acos",Blockly.Msg.MATH_TRIG_ATAN="atan",Blockly.Msg.MATH_ARITHMETIC_HELPURL="https://en.wikipedia.org/wiki/Arithmetic",Blockly.Msg.MATH_ARITHMETIC_TOOLTIP_ADD="Return the sum of the two numbers.",Blockly.Msg.MATH_ARITHMETIC_TOOLTIP_MINUS="Return the difference of the two numbers.",Blockly.Msg.MATH_ARITHMETIC_TOOLTIP_MULTIPLY="Return the product of the two numbers.",Blockly.Msg.MATH_ARITHMETIC_TOOLTIP_DIVIDE="Return the quotient of the two numbers.",Blockly.Msg.MATH_ARITHMETIC_TOOLTIP_POWER="Return the first number raised to the power of the second number.",Blockly.Msg.MATH_SINGLE_HELPURL="https://en.wikipedia.org/wiki/Square_root",Blockly.Msg.MATH_SINGLE_OP_ROOT="square root",Blockly.Msg.MATH_SINGLE_TOOLTIP_ROOT="Return the square root of a number.",Blockly.Msg.MATH_SINGLE_OP_ABSOLUTE="absolute",Blockly.Msg.MATH_SINGLE_TOOLTIP_ABS="Return the absolute value of a number.",Blockly.Msg.MATH_SINGLE_TOOLTIP_NEG="Return the negation of a number.",Blockly.Msg.MATH_SINGLE_TOOLTIP_LN="Return the natural logarithm of a number.",Blockly.Msg.MATH_SINGLE_TOOLTIP_LOG10="Return the base 10 logarithm of a number.",Blockly.Msg.MATH_SINGLE_TOOLTIP_EXP="Return e to the power of a number.",Blockly.Msg.MATH_SINGLE_TOOLTIP_POW10="Return 10 to the power of a number.",Blockly.Msg.MATH_TRIG_HELPURL="https://en.wikipedia.org/wiki/Trigonometric_functions",Blockly.Msg.MATH_TRIG_TOOLTIP_SIN="Return the sine of a degree (not radian).",Blockly.Msg.MATH_TRIG_TOOLTIP_COS="Return the cosine of a degree (not radian).",Blockly.Msg.MATH_TRIG_TOOLTIP_TAN="Return the tangent of a degree (not radian).",Blockly.Msg.MATH_TRIG_TOOLTIP_ASIN="Return the arcsine of a number.",Blockly.Msg.MATH_TRIG_TOOLTIP_ACOS="Return the arccosine of a number.",Blockly.Msg.MATH_TRIG_TOOLTIP_ATAN="Return the arctangent of a number.",Blockly.Msg.MATH_CONSTANT_HELPURL="https://en.wikipedia.org/wiki/Mathematical_constant",Blockly.Msg.MATH_CONSTANT_TOOLTIP="Return one of the common constants: π (3.141…), e (2.718…), φ (1.618…), sqrt(2) (1.414…), sqrt(½) (0.707…), or ∞ (infinity).",Blockly.Msg.MATH_IS_EVEN="is even",Blockly.Msg.MATH_IS_ODD="is odd",Blockly.Msg.MATH_IS_PRIME="is prime",Blockly.Msg.MATH_IS_WHOLE="is whole",Blockly.Msg.MATH_IS_POSITIVE="is positive",Blockly.Msg.MATH_IS_NEGATIVE="is negative",Blockly.Msg.MATH_IS_DIVISIBLE_BY="is divisible by",Blockly.Msg.MATH_IS_TOOLTIP="Check if a number is an even, odd, prime, whole, positive, negative, or if it is divisible by certain number.  Returns true or false.",Blockly.Msg.MATH_CHANGE_HELPURL="https://en.wikipedia.org/wiki/Programming_idiom#Incrementing_a_counter",Blockly.Msg.MATH_CHANGE_TITLE_CHANGE="change",Blockly.Msg.MATH_CHANGE_TITLE_ITEM=Blockly.Msg.VARIABLES_DEFAULT_NAME,Blockly.Msg.MATH_CHANGE_INPUT_BY="by",Blockly.Msg.MATH_CHANGE_TOOLTIP='Add a number to variable "%1".',Blockly.Msg.MATH_ROUND_HELPURL="https://en.wikipedia.org/wiki/Rounding",Blockly.Msg.MATH_ROUND_TOOLTIP="Round a number up or down.",Blockly.Msg.MATH_ROUND_OPERATOR_ROUND="round",Blockly.Msg.MATH_ROUND_OPERATOR_ROUNDUP="round up",Blockly.Msg.MATH_ROUND_OPERATOR_ROUNDDOWN="round down",Blockly.Msg.MATH_ONLIST_HELPURL="",Blockly.Msg.MATH_ONLIST_OPERATOR_SUM="sum of list",Blockly.Msg.MATH_ONLIST_TOOLTIP_SUM="Return the sum of all the numbers in the list.",Blockly.Msg.MATH_ONLIST_OPERATOR_MIN="min of list",Blockly.Msg.MATH_ONLIST_TOOLTIP_MIN="Return the smallest number in the list.",Blockly.Msg.MATH_ONLIST_OPERATOR_MAX="max of list",Blockly.Msg.MATH_ONLIST_TOOLTIP_MAX="Return the largest number in the list.",Blockly.Msg.MATH_ONLIST_OPERATOR_AVERAGE="average of list",Blockly.Msg.MATH_ONLIST_TOOLTIP_AVERAGE="Return the average (arithmetic mean) of the numeric values in the list.",Blockly.Msg.MATH_ONLIST_OPERATOR_MEDIAN="median of list",Blockly.Msg.MATH_ONLIST_TOOLTIP_MEDIAN="Return the median number in the list.",Blockly.Msg.MATH_ONLIST_OPERATOR_MODE="modes of list",Blockly.Msg.MATH_ONLIST_TOOLTIP_MODE="Return a list of the most common item(s) in the list.",Blockly.Msg.MATH_ONLIST_OPERATOR_STD_DEV="standard deviation of list",Blockly.Msg.MATH_ONLIST_TOOLTIP_STD_DEV="Return the standard deviation of the list.",Blockly.Msg.MATH_ONLIST_OPERATOR_RANDOM="random item of list",Blockly.Msg.MATH_ONLIST_TOOLTIP_RANDOM="Return a random element from the list.",Blockly.Msg.MATH_MODULO_HELPURL="https://en.wikipedia.org/wiki/Modulo_operation",Blockly.Msg.MATH_MODULO_TITLE="remainder of %1 ÷ %2",Blockly.Msg.MATH_MODULO_TOOLTIP="Return the remainder from dividing the two numbers.",Blockly.Msg.MATH_CONSTRAIN_HELPURL="https://en.wikipedia.org/wiki/Clamping_%28graphics%29",Blockly.Msg.MATH_CONSTRAIN_TITLE="constrain %1 low %2 high %3",Blockly.Msg.MATH_CONSTRAIN_TOOLTIP="Constrain a number to be between the specified limits (inclusive).",Blockly.Msg.MATH_RANDOM_INT_HELPURL="https://en.wikipedia.org/wiki/Random_number_generation",Blockly.Msg.MATH_RANDOM_INT_TITLE="random integer from %1 to %2",Blockly.Msg.MATH_RANDOM_INT_TOOLTIP="Return a random integer between the two specified limits, inclusive.",Blockly.Msg.MATH_RANDOM_FLOAT_HELPURL="https://en.wikipedia.org/wiki/Random_number_generation",Blockly.Msg.MATH_RANDOM_FLOAT_TITLE_RANDOM="random fraction",Blockly.Msg.MATH_RANDOM_FLOAT_TOOLTIP="Return a random fraction between 0.0 (inclusive) and 1.0 (exclusive).",Blockly.Msg.TEXT_TEXT_HELPURL="https://en.wikipedia.org/wiki/String_(computer_science)",Blockly.Msg.TEXT_TEXT_TOOLTIP="A letter, word, or line of text.",Blockly.Msg.TEXT_JOIN_HELPURL="https://code.google.com/p/blockly/wiki/Text#Text_creation",Blockly.Msg.TEXT_JOIN_TITLE_CREATEWITH="create text with",Blockly.Msg.TEXT_JOIN_TOOLTIP="Create a piece of text by joining together any number of items.",Blockly.Msg.TEXT_CREATE_JOIN_TITLE_JOIN="join",Blockly.Msg.TEXT_CREATE_JOIN_TOOLTIP="Add, remove, or reorder sections to reconfigure this text block.",Blockly.Msg.TEXT_CREATE_JOIN_ITEM_TITLE_ITEM=Blockly.Msg.VARIABLES_DEFAULT_NAME,Blockly.Msg.TEXT_CREATE_JOIN_ITEM_TOOLTIP="Add an item to the text.",Blockly.Msg.TEXT_APPEND_HELPURL="https://code.google.com/p/blockly/wiki/Text#Text_modification",Blockly.Msg.TEXT_APPEND_TO="to",Blockly.Msg.TEXT_APPEND_APPENDTEXT="append text",Blockly.Msg.TEXT_APPEND_VARIABLE=Blockly.Msg.VARIABLES_DEFAULT_NAME,Blockly.Msg.TEXT_APPEND_TOOLTIP='Append some text to variable "%1".',Blockly.Msg.TEXT_LENGTH_HELPURL="https://code.google.com/p/blockly/wiki/Text#Text_modification",Blockly.Msg.TEXT_LENGTH_TITLE="length of %1",Blockly.Msg.TEXT_LENGTH_TOOLTIP="Returns the number of letters (including spaces) in the provided text.",Blockly.Msg.TEXT_ISEMPTY_HELPURL="https://code.google.com/p/blockly/wiki/Text#Checking_for_empty_text",Blockly.Msg.TEXT_ISEMPTY_TITLE="%1 is empty",Blockly.Msg.TEXT_ISEMPTY_TOOLTIP="Returns true if the provided text is empty.",Blockly.Msg.TEXT_INDEXOF_HELPURL="https://code.google.com/p/blockly/wiki/Text#Finding_text",Blockly.Msg.TEXT_INDEXOF_TOOLTIP="Returns the index of the first/last occurrence of first text in the second text.  Returns 0 if text is not found.",Blockly.Msg.TEXT_INDEXOF_INPUT_INTEXT="in text",Blockly.Msg.TEXT_INDEXOF_OPERATOR_FIRST="find first occurrence of text",Blockly.Msg.TEXT_INDEXOF_OPERATOR_LAST="find last occurrence of text",Blockly.Msg.TEXT_INDEXOF_TAIL="",Blockly.Msg.TEXT_CHARAT_HELPURL="https://code.google.com/p/blockly/wiki/Text#Extracting_text",Blockly.Msg.TEXT_CHARAT_INPUT_INTEXT="in text",Blockly.Msg.TEXT_CHARAT_FROM_START="get letter #",Blockly.Msg.TEXT_CHARAT_FROM_END="get letter # from end",Blockly.Msg.TEXT_CHARAT_FIRST="get first letter",Blockly.Msg.TEXT_CHARAT_LAST="get last letter",Blockly.Msg.TEXT_CHARAT_RANDOM="get random letter",Blockly.Msg.TEXT_CHARAT_TAIL="",Blockly.Msg.TEXT_CHARAT_TOOLTIP="Returns the letter at the specified position.",Blockly.Msg.TEXT_GET_SUBSTRING_TOOLTIP="Returns a specified portion of the text.",Blockly.Msg.TEXT_GET_SUBSTRING_HELPURL="https://code.google.com/p/blockly/wiki/Text#Extracting_a_region_of_text",Blockly.Msg.TEXT_GET_SUBSTRING_INPUT_IN_TEXT="in text",Blockly.Msg.TEXT_GET_SUBSTRING_START_FROM_START="get substring from letter #",Blockly.Msg.TEXT_GET_SUBSTRING_START_FROM_END="get substring from letter # from end",Blockly.Msg.TEXT_GET_SUBSTRING_START_FIRST="get substring from first letter",Blockly.Msg.TEXT_GET_SUBSTRING_END_FROM_START="to letter #",Blockly.Msg.TEXT_GET_SUBSTRING_END_FROM_END="to letter # from end",Blockly.Msg.TEXT_GET_SUBSTRING_END_LAST="to last letter",Blockly.Msg.TEXT_GET_SUBSTRING_TAIL="",Blockly.Msg.TEXT_CHANGECASE_HELPURL="https://code.google.com/p/blockly/wiki/Text#Adjusting_text_case",Blockly.Msg.TEXT_CHANGECASE_TOOLTIP="Return a copy of the text in a different case.",Blockly.Msg.TEXT_CHANGECASE_OPERATOR_UPPERCASE="to UPPER CASE",Blockly.Msg.TEXT_CHANGECASE_OPERATOR_LOWERCASE="to lower case",Blockly.Msg.TEXT_CHANGECASE_OPERATOR_TITLECASE="to Title Case",Blockly.Msg.TEXT_TRIM_HELPURL="https://code.google.com/p/blockly/wiki/Text#Trimming_%28removing%29_spaces",Blockly.Msg.TEXT_TRIM_TOOLTIP="Return a copy of the text with spaces removed from one or both ends.",Blockly.Msg.TEXT_TRIM_OPERATOR_BOTH="trim spaces from both sides of",Blockly.Msg.TEXT_TRIM_OPERATOR_LEFT="trim spaces from left side of",Blockly.Msg.TEXT_TRIM_OPERATOR_RIGHT="trim spaces from right side of",Blockly.Msg.TEXT_PRINT_HELPURL="https://code.google.com/p/blockly/wiki/Text#Printing_text",Blockly.Msg.TEXT_PRINT_TITLE="print %1",Blockly.Msg.TEXT_PRINT_TOOLTIP="Print the specified text, number or other value.",Blockly.Msg.TEXT_PROMPT_HELPURL="https://code.google.com/p/blockly/wiki/Text#Getting_input_from_the_user",Blockly.Msg.TEXT_PROMPT_TYPE_TEXT="prompt for text with message",Blockly.Msg.TEXT_PROMPT_TYPE_NUMBER="prompt for number with message",Blockly.Msg.TEXT_PROMPT_TOOLTIP_NUMBER="Prompt for user for a number.",Blockly.Msg.TEXT_PROMPT_TOOLTIP_TEXT="Prompt for user for some text.",Blockly.Msg.LISTS_CREATE_EMPTY_HELPURL="https://en.wikipedia.org/wiki/Linked_list#Empty_lists",Blockly.Msg.LISTS_CREATE_EMPTY_TITLE="create empty list",Blockly.Msg.LISTS_CREATE_EMPTY_TOOLTIP="Returns a list, of length 0, containing no data records",Blockly.Msg.LISTS_CREATE_WITH_TOOLTIP="Create a list with any number of items.",Blockly.Msg.LISTS_CREATE_WITH_INPUT_WITH="create list with",Blockly.Msg.LISTS_CREATE_WITH_CONTAINER_TITLE_ADD="list",Blockly.Msg.LISTS_CREATE_WITH_CONTAINER_TOOLTIP="Add, remove, or reorder sections to reconfigure this list block.",Blockly.Msg.LISTS_CREATE_WITH_ITEM_TITLE=Blockly.Msg.VARIABLES_DEFAULT_NAME,Blockly.Msg.LISTS_CREATE_WITH_ITEM_TOOLTIP="Add an item to the list.",Blockly.Msg.LISTS_REPEAT_HELPURL="https://code.google.com/p/blockly/wiki/Lists#create_list_with",Blockly.Msg.LISTS_REPEAT_TOOLTIP="Creates a list consisting of the given value repeated the specified number of times.",Blockly.Msg.LISTS_REPEAT_TITLE="create list with item %1 repeated %2 times",Blockly.Msg.LISTS_LENGTH_HELPURL="https://code.google.com/p/blockly/wiki/Lists#length_of",Blockly.Msg.LISTS_LENGTH_TITLE="length of %1",Blockly.Msg.LISTS_LENGTH_TOOLTIP="Returns the length of a list.",Blockly.Msg.LISTS_IS_EMPTY_HELPURL="https://code.google.com/p/blockly/wiki/Lists#is_empty",Blockly.Msg.LISTS_IS_EMPTY_TITLE="%1 is empty",Blockly.Msg.LISTS_TOOLTIP="Returns true if the list is empty.",Blockly.Msg.LISTS_INLIST="in list",Blockly.Msg.LISTS_INDEX_OF_HELPURL="https://code.google.com/p/blockly/wiki/Lists#Getting_Items_from_a_List",Blockly.Msg.LISTS_INDEX_OF_INPUT_IN_LIST=Blockly.Msg.LISTS_INLIST,Blockly.Msg.LISTS_INDEX_OF_FIRST="find first occurrence of item",Blockly.Msg.LISTS_INDEX_OF_LAST="find last occurrence of item",Blockly.Msg.LISTS_INDEX_OF_TOOLTIP="Returns the index of the first/last occurrence of the item in the list.  Returns 0 if text is not found.",Blockly.Msg.LISTS_GET_INDEX_HELPURL=Blockly.Msg.LISTS_INDEX_OF_HELPURL,Blockly.Msg.LISTS_GET_INDEX_GET="get",Blockly.Msg.LISTS_GET_INDEX_GET_REMOVE="get and remove",Blockly.Msg.LISTS_GET_INDEX_REMOVE="remove",Blockly.Msg.LISTS_GET_INDEX_FROM_START="#",Blockly.Msg.LISTS_GET_INDEX_FROM_END="# from end",Blockly.Msg.LISTS_GET_INDEX_FIRST="first",Blockly.Msg.LISTS_GET_INDEX_LAST="last",Blockly.Msg.LISTS_GET_INDEX_RANDOM="random",Blockly.Msg.LISTS_GET_INDEX_TAIL="",Blockly.Msg.LISTS_GET_INDEX_INPUT_IN_LIST=Blockly.Msg.LISTS_INLIST,Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_GET_FROM_START="Returns the item at the specified position in a list.  #1 is the first item.",Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_GET_FROM_END="Returns the item at the specified position in a list.  #1 is the last item.",Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_GET_FIRST="Returns the first item in a list.",Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_GET_LAST="Returns the last item in a list.",Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_GET_RANDOM="Returns a random item in a list.",Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_GET_REMOVE_FROM_START="Removes and returns the item at the specified position in a list.  #1 is the first item.",Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_GET_REMOVE_FROM_END="Removes and returns the item at the specified position in a list.  #1 is the last item.",Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_GET_REMOVE_FIRST="Removes and returns the first item in a list.",Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_GET_REMOVE_LAST="Removes and returns the last item in a list.",Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_GET_REMOVE_RANDOM="Removes and returns a random item in a list.",Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_REMOVE_FROM_START="Removes the item at the specified position in a list.  #1 is the first item.",Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_REMOVE_FROM_END="Removes the item at the specified position in a list.  #1 is the last item.",Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_REMOVE_FIRST="Removes the first item in a list.",Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_REMOVE_LAST="Removes the last item in a list.",Blockly.Msg.LISTS_GET_INDEX_TOOLTIP_REMOVE_RANDOM="Removes a random item in a list.",Blockly.Msg.LISTS_SET_INDEX_HELPURL="https://code.google.com/p/blockly/wiki/Lists#in_list_..._set",Blockly.Msg.LISTS_SET_INDEX_INPUT_IN_LIST=Blockly.Msg.LISTS_INLIST,Blockly.Msg.LISTS_SET_INDEX_SET="set",Blockly.Msg.LISTS_SET_INDEX_INSERT="insert at",Blockly.Msg.LISTS_SET_INDEX_INPUT_TO="as",Blockly.Msg.LISTS_SET_INDEX_TOOLTIP_SET_FROM_START="Sets the item at the specified position in a list.  #1 is the first item.",Blockly.Msg.LISTS_SET_INDEX_TOOLTIP_SET_FROM_END="Sets the item at the specified position in a list.  #1 is the last item.",Blockly.Msg.LISTS_SET_INDEX_TOOLTIP_SET_FIRST="Sets the first item in a list.",Blockly.Msg.LISTS_SET_INDEX_TOOLTIP_SET_LAST="Sets the last item in a list.",Blockly.Msg.LISTS_SET_INDEX_TOOLTIP_SET_RANDOM="Sets a random item in a list.",Blockly.Msg.LISTS_SET_INDEX_TOOLTIP_INSERT_FROM_START="Inserts the item at the specified position in a list.  #1 is the first item.",Blockly.Msg.LISTS_SET_INDEX_TOOLTIP_INSERT_FROM_END="Inserts the item at the specified position in a list.  #1 is the last item.",Blockly.Msg.LISTS_SET_INDEX_TOOLTIP_INSERT_FIRST="Inserts the item at the start of a list.",Blockly.Msg.LISTS_SET_INDEX_TOOLTIP_INSERT_LAST="Append the item to the end of a list.",Blockly.Msg.LISTS_SET_INDEX_TOOLTIP_INSERT_RANDOM="Inserts the item randomly in a list.",Blockly.Msg.LISTS_GET_SUBLIST_HELPURL="https://code.google.com/p/blockly/wiki/Lists#Getting_a_sublist",Blockly.Msg.LISTS_GET_SUBLIST_INPUT_IN_LIST=Blockly.Msg.LISTS_INLIST,Blockly.Msg.LISTS_GET_SUBLIST_START_FROM_START="get sub-list from #",Blockly.Msg.LISTS_GET_SUBLIST_START_FROM_END="get sub-list from # from end",Blockly.Msg.LISTS_GET_SUBLIST_START_FIRST="get sub-list from first",Blockly.Msg.LISTS_GET_SUBLIST_END_FROM_START="to #",Blockly.Msg.LISTS_GET_SUBLIST_END_FROM_END="to # from end",Blockly.Msg.LISTS_GET_SUBLIST_END_LAST="to last",Blockly.Msg.LISTS_GET_SUBLIST_TAIL="",Blockly.Msg.LISTS_GET_SUBLIST_TOOLTIP="Creates a copy of the specified portion of a list.",Blockly.Msg.ORDINAL_NUMBER_SUFFIX="",Blockly.Msg.VARIABLES_GET_HELPURL="https://code.google.com/p/blockly/wiki/Variables#Get",Blockly.Msg.VARIABLES_GET_TITLE="",Blockly.Msg.VARIABLES_GET_ITEM=Blockly.Msg.VARIABLES_DEFAULT_NAME,Blockly.Msg.VARIABLES_GET_TAIL="",Blockly.Msg.VARIABLES_GET_TOOLTIP="Returns the value of this variable.",Blockly.Msg.VARIABLES_GET_CREATE_SET='Create "set %1"',Blockly.Msg.VARIABLES_SET_HELPURL="https://code.google.com/p/blockly/wiki/Variables#Set",Blockly.Msg.VARIABLES_SET_TITLE="set",Blockly.Msg.VARIABLES_SET_ITEM=Blockly.Msg.VARIABLES_DEFAULT_NAME,Blockly.Msg.VARIABLES_SET_TAIL="to",Blockly.Msg.VARIABLES_SET_TOOLTIP="Sets this variable to be equal to the input.",Blockly.Msg.VARIABLES_SET_CREATE_GET='Create "get %1"',Blockly.Msg.PROCEDURES_DEFNORETURN_HELPURL="https://en.wikipedia.org/wiki/Procedure_%28computer_science%29",Blockly.Msg.PROCEDURES_DEFNORETURN_TITLE="to",Blockly.Msg.PROCEDURES_DEFNORETURN_PROCEDURE="do something",Blockly.Msg.PROCEDURES_BEFORE_PARAMS="with:",Blockly.Msg.PROCEDURES_DEFNORETURN_DO="",Blockly.Msg.PROCEDURES_DEFNORETURN_TOOLTIP="Creates a function with no output.",Blockly.Msg.PROCEDURES_DEFRETURN_HELPURL="https://en.wikipedia.org/wiki/Procedure_%28computer_science%29",Blockly.Msg.PROCEDURES_DEFRETURN_TITLE=Blockly.Msg.PROCEDURES_DEFNORETURN_TITLE,Blockly.Msg.PROCEDURES_DEFRETURN_PROCEDURE=Blockly.Msg.PROCEDURES_DEFNORETURN_PROCEDURE,Blockly.Msg.PROCEDURES_DEFRETURN_DO=Blockly.Msg.PROCEDURES_DEFNORETURN_DO,Blockly.Msg.PROCEDURES_DEFRETURN_RETURN="return",Blockly.Msg.PROCEDURES_DEFRETURN_TOOLTIP="Creates a function with an output.",Blockly.Msg.PROCEDURES_DEF_DUPLICATE_WARNING="Warning: This function has duplicate parameters.",Blockly.Msg.PROCEDURES_CALLNORETURN_HELPURL="https://en.wikipedia.org/wiki/Procedure_%28computer_science%29",Blockly.Msg.PROCEDURES_CALLNORETURN_CALL="",Blockly.Msg.PROCEDURES_CALLNORETURN_TOOLTIP='Run the user-defined function "%1".',Blockly.Msg.PROCEDURES_CALLRETURN_HELPURL="https://en.wikipedia.org/wiki/Procedure_%28computer_science%29",Blockly.Msg.PROCEDURES_CALLRETURN_CALL=Blockly.Msg.PROCEDURES_CALLNORETURN_CALL,Blockly.Msg.PROCEDURES_CALLRETURN_TOOLTIP='Run the user-defined function "%1" and use its output.',Blockly.Msg.PROCEDURES_MUTATORCONTAINER_TITLE="inputs",Blockly.Msg.PROCEDURES_MUTATORCONTAINER_TOOLTIP="Add, remove, or reorder inputs to this function.",Blockly.Msg.PROCEDURES_MUTATORARG_TITLE="input name:",Blockly.Msg.PROCEDURES_MUTATORARG_TOOLTIP="Add an input to the function.",Blockly.Msg.PROCEDURES_HIGHLIGHT_DEF="Highlight function definition",Blockly.Msg.PROCEDURES_CREATE_DO='Create "%1"',Blockly.Msg.PROCEDURES_IFRETURN_TOOLTIP="If a value is true, then return a second value.",Blockly.Msg.PROCEDURES_IFRETURN_WARNING="Warning: This block may be used only within a function definition.";
var doc=document;doc.el=doc.getElementById;var unclass=goog.dom.classes.remove,addclass=goog.dom.classes.add;Blockly.inject(doc.el("content_BLL"),{path:"./",toolbox:doc.el("toolbox"),scrollbars:!0,trashcan:!1}),window.setTimeout(BlocklyStorage.restoreBlocks,0),BlocklyStorage.backupOnUnload();
